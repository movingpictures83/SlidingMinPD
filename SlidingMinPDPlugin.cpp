...
void SlidingMinPDPlugin::freeBKPNode(BKPNode *bkpLL)
{
	BKPNode *Navigate;
	while(bkpLL!=NULL){
		Navigate=bkpLL;
...
int SlidingMinPDPlugin::BitCriteria(int Num, int Bit){/* Is bit set in Num */
	int i;
	int PowBit;
...
int SlidingMinPDPlugin::TwoCrossovers(int Num, int MaxBit, int *Begin, int *End){/* Is bit set in Num */
	int i;
	int Start,Stop;
	Stop=-1;
	Start=-1;
...
double SlidingMinPDPlugin::SumofSquaredValues(double ***values, int **values2, int points, int data, int s,double *m_x)
{
	double Sx;
	int t;
		(*m_x)=0;
...
char **  SlidingMinPDPlugin::MatrixInit(int s, int dim1, int dim2)
{
	char  **matrix  = NULL;
	int   i;
...
/*********************************************************************/
double SlidingMinPDPlugin::JC69distance(int gaps, int matches, int al_len, double alpha)
{
...
/*********************************************************************/
double SlidingMinPDPlugin::K2P80distance(int p1, int p2, int gaps, int matches, int al_len, double alpha)
{
...
/*********************************************************************/
double SlidingMinPDPlugin::TN93distance(int pT, int pC, int pG, int pA, int p1, int p2, int gaps,int matches, int al_len, double alpha, int s, int t)
{
...
/*  					Calculate pairwise			  							          */
/******************************************************************************************/
double SlidingMinPDPlugin::PairwiseDistance(char **seqsChars,int t, int s, int windowSize, int model)
{
	int i;
	char c1,c2;
...
/*  																  */
/**********************************************************************/
int SlidingMinPDPlugin::DistOnly(Fasta **seqs, double **dist, double ***dist_frags, int maxDim, int n,int win_Count, int model, int windowSize, int step, int numWins)
{
	char		**seqChars;
...
/*  																  */
/**********************************************************************/
int SlidingMinPDPlugin::SaveMinResults(Mintaxa **minresults, int *resno, int a, int c, double dist, double div)
{
...
/*  																  */
/**********************************************************************/
void SlidingMinPDPlugin::OptionalRecPrintout(FILE *fp4,  RecRes *recRes,Fasta **seqs,int *Frag_Seq,FDisNode (*recSolutions)[3][2], int i, int l, int r, int p, int q, int f, int index, int last, int BKP, int win_Count,int align,int fr_size)
{
	double d_min;
	d_min=INF9999;
...
/* 							FillArrayInOrder					      */
/**********************************************************************/
double SlidingMinPDPlugin::FillArrayInOrder(int *arrayInOrder, double ***dist_frags, double **bootVals, int  *Frag_Seq, int s, int k, int noSeqs, int bootOpt)
{
	int t,j,p,q,i;
...
BKPNode * SlidingMinPDPlugin::ArrayToBKPNode( int  *F, int win_Count, int fr_size, 
...
/*  																  */
/**********************************************************************/
BKPNode * SlidingMinPDPlugin::GetRecSolutionsII( double ***dist_frags, double **bootVals, Fasta **seqs, int  *Frag_Seq, int noSeqs, int s, int win_Count, int min_seq, int fr_size, double min_seq_dist, int align, int bootOpt, int step, int *avgBS) {
...
/**********************************************************************/
double SlidingMinPDPlugin::FindBestinStretch( double ***dist_frags, double **bootVals, int  *Frag_Seq,   int bootOpt,int noSeqs, int s, int win_Count, int candidate,int lb,int ub,double largest)
{
	double d;
	int i;
			d=0;
...
/*  																  */
/**********************************************************************/
BKPNode * SlidingMinPDPlugin::GetRecSolutionsI(double ***dist_frags, double **bootVals, Fasta **seqs, int  *Frag_Seq,  int noSeqs, int s, int win_Count, int min_seq, int fr_size, double min_seq_dist, int align, int bootOpt, int step,int *avgBS)
{
...
/*  																  */
/**********************************************************************/
int SlidingMinPDPlugin::PickSeqofLargerAvgDis(double ***dist_frags, int k_idx, int f_idx, int min_idx,  int s, int win_Count, int k, int f,double min_d)
{
...
int SlidingMinPDPlugin::PickSeqMinDist(double ***dist_frags, int  *Frag_Seq, int min_idx,int noSeqs, int k_idx, int f_idx, int s, int win_Count, int k, int f,double min_d)
{
...
/* 						Print Fragments							      */
/**********************************************************************/
void SlidingMinPDPlugin::PrintFragments(FILE *fp3, int  *Frag_Seq, Fasta **seqs, double **dist, double ***dist_frags, int win_Count, int i, int s, int align )
{
	int t,k;
...
/**********************************************************************/
int SlidingMinPDPlugin::PickCandidates( Fasta **seqs, int  *Frag_Seq, double ***dist_frags, int win_Count,
...
BKPNode * SlidingMinPDPlugin::Check4Recombination(FILE *fp3, FILE *fp4, int  *Frag_Seq,  Fasta **seqs, double **dist, double ***dist_frags, int startAS, int s, int win_Count,  int min_idx,  int align, int maxDim, int opt, 
...
double SlidingMinPDPlugin::GetBootstrapValues(Fasta **seqs, double ***bootVals,  int s, int a, int bootreps)
{
...
double SlidingMinPDPlugin::GetClusteredBootstrapValues(Mintaxa *mintaxa, int max_times, Fasta **seqs, double ***bootVals,  int s, int min_idx, int bootreps)
{
...
/**********************************************************************/
int SlidingMinPDPlugin::CheckClustering(Mintaxa *mintaxa,int max_times,Fasta **seqs,int s,int min_idx)
{
...
/**********************************************************************/
void SlidingMinPDPlugin::StoreBootstrapValues(int *boots, Mintaxa **minresults, int resno, int start)
{
	int i,j;
...
/*  																  */
/**********************************************************************/
int SlidingMinPDPlugin::OutputResults(int win_Count, int s, int startAS, double iBase, double min_d, int min_idx, int *times_max, double *div, BKPNode *bkpLL, double ***bootVals, Mintaxa *mintaxa,Mintaxa **minresults, Fasta **seqs, double **dist, int maxDim, int n, FILE *fp1, double mutrate, int fr_size, int *resno, int align, int avgBS,int bootreps)
{
	double	 d,  interval;
	int		i,  j, t,times; 
...
/*  																  */
/**********************************************************************/
int SlidingMinPDPlugin::GetMinDist(Mintaxa **minresults, Fasta **seqs, double **dist, double ***dist_frags, int maxDim, int n, char *File1, char *File2, double mutrate, int win_Count, int *resno, int align, int opt, int step, int winSize)
{
...
void SlidingMinPDPlugin::BuildPartialNJTrees(Fasta **seqs, Mintaxa **minresults, int resno, double **dist, char *File1, int n)
{
	FILE	*fp1;
	int		r, ch, i,j,a, w;
	int		max_w=0,ftime;
...
/******************************************************************************************/
int SlidingMinPDPlugin::PrepareWeights(int *wmod,int bootreps, int windowSize)
{
	double rnd;
...
int SlidingMinPDPlugin::BootRip(short int bootreps,  int model, double alpha, int noSeqs, int windowSize, char *seqs,  double *dist,  int *wmod)
{
...
//}
		...
		for (s=1;s<noSeqs;s++){
			...
			//	printf("stop\n");
				...
				/*else {
					printf("OK distVal=%lf, s=%d, t=%d, b=%d \n",distVal,s,t,b);
...
int SlidingMinPDPlugin::GetBootNJdistances(int bootreps,int noSeqs, double *dstMat,Fasta **seqs, int distPenalty)
{
...
int SlidingMinPDPlugin::DoBootscan(double ***dist_frags,int **plotVal,int numWins, Fasta **seqsBS,  int maxDim, int noSeqs,int stepSize, int windowSize,int closestRel, int opt, short int bootreps, int fullBootscan)
{
	//short int *seqsInt;
	short int full;
	char *seqChars;
...
/******************************************************************************************/
 ...
 int SlidingMinPDPlugin::BootscanShrinkPool( FILE *fp4, Fasta **seqs, int  *Frag_Seq, int step, int windowSize , short int bootReps,
...
BKPNode * SlidingMinPDPlugin::BootscanCandidates(FILE *fp4,  Fasta **seqs, int  *Frag_Seq, int step, int windowSize , short int bootReps,
...
/******************************************************************************************/
int SlidingMinPDPlugin::CopyToSeqsBootscan(Fasta **seqsBS, Fasta **seqs,int numSeqs, int seqNo, int *maxBSDim)
{
	//int i=0;
...
int SlidingMinPDPlugin::GetParentsFromAnswer(Fasta **seqs,char *temp,int *parents, int n)
{
	int k, i,p;
	char name[20];
...
int SlidingMinPDPlugin::Parents_Missed(int *Frag_Seq,int numSeqs,int *parents,int p)
{
	int i,j,pa[10],k;
	for(i=0;i<p;i++)
		pa[i]=false;
...
int SlidingMinPDPlugin::GetAvgBootstrapDist(Fasta **seqs,double **dist, double ***bootVals, int n, int bootreps)
{
...
int SlidingMinPDPlugin::Bootscan(char *File1, char *File2, double ***dist_frags, int numWins, Mintaxa **minresults,Fasta **seqs, double **dist, int maxDim, int n,int step, int windowSize, int closestRel, int *resno, short int bootreps, int align)
{
...
/**************************************************************/
void SlidingMinPDPlugin::ReadUntil(FILE *fv, char stopChar, char *what)
{
	char ch;
...
void SlidingMinPDPlugin::PrintUsage()
{
...
void SlidingMinPDPlugin::ReadParams( char *inputFile, char *outputFile, int *opt, int *align,  int *wSize, int *stSize, char* ifile)
{
	...
	*align=0;
	...
	/*if(feof(fpIn)){
		fprintf(stderr, "Unable to read parameters from fpIn\n");
		exit(0);
	}*/
	//str2="BEGIN TVBLOCK";
	fgets(str, 255, fpIn);
	...
	while(isspace(ch))
		ch=fgetc(fpIn);
	while(ch=='['){
		ReadUntil(fpIn, ']', "closing bracket");
		ch=fgetc(fpIn);
		while(isspace(ch))
			ch=fgetc(fpIn);
	}
	...
	while(!feof(fpIn)){
		...
		switch (ch) {
			...
			case 'B':
				if (fscanf(fpIn, "%d", &Bootstrap)!=1 || (Bootstrap!=0 && Bootstrap!=1)) {
					fprintf(stderr, "Bad [Bootstrap:1,Jackknife:0] value. Must be 0 or 1.\n");
					PrintUsage();
				}
			break;
			case 'C':
				if (fscanf(fpIn, "%d", &crossOpt)!=1 || crossOpt<0 ||crossOpt>1) {
					fprintf(stderr, "Bad [crossover option] number. Must be 0 or 1.\n");
					PrintUsage();
				}
			break;
			case 'D':
				if (fscanf(fpIn, "%d", &reportDistances)!=1 || reportDistances<0 ||reportDistances>1) {
					fprintf(stderr, "Bad [report all distances] number. Must be 0 or 1.\n");
					PrintUsage();
				}
			break;
			case 'E':
				if (fscanf(fpIn, "%d", &seed)!=1 ) {
					fprintf(stderr, "Bad [bootscan seed] number\n");
					PrintUsage();
				}
			break;
			case 'F':
				if (fscanf(fpIn, "%d", &recOn)!=1 || recOn<0  || recOn>1 ) {
			...
			break;				
			case 'H': 
				if (fscanf(fpIn, "%d", &BootThreshold)!=1 || BootThreshold<50 ||BootThreshold>100 ) {
			...
			case 'K':
				if (fscanf(fpIn, "%d", &printBoot)!=1 || (printBoot!=0 && printBoot!=1)) {
					fprintf(stderr, "Bad [show bootstrap values] option. Must be 0 or 1.\n");
					PrintUsage();
				}
			break;
			case 'L': 
				if (fscanf(fpIn, "%d", &closestRel)!=1 ) {
			...
			break;
			case 'P':
				if (fscanf(fpIn, "%lf", &PCCThreshold)!=1|| PCCThreshold<0.0 || PCCThreshold>1.0) {
					fprintf(stderr, "Bad [PCC threshold] value\n\n");
					PrintUsage();
				}
			break;
			case 'R':
				if (fscanf(fpIn, "%d", opt)!=1) {
					fprintf(stderr, "Bad [recombination detection] value\n\n");
					PrintUsage();
				}
			break;
			case 'S':
				if (fscanf(fpIn, "%d", stSize)!=1 ||*wSize<50 ||*stSize>*wSize || *stSize<10 ) {
					if(*opt==2){
			...
			break;
			case 'T':
				if (fscanf(fpIn, "%d", &BootTieBreaker)!=1 || BootTieBreaker<0  || BootTieBreaker>1 ) {
			...
			break;
			case 'W':
				if (fscanf(fpIn, "%d", wSize)!=1 && *opt==2 ) {
...
void SlidingMinPDPlugin::CheckTimes(Fasta **seqs, int n){
int i, time;
time=seqs[0]->time;
	for (i=1;i<n;i++){
		if(time!=seqs[i]->time){
...
void SlidingMinPDPlugin::input(std::string infile){
	GOP=5; //+ log(len2);
	GEP=1;
	match = 2;
	mismatch=0;
...
void SlidingMinPDPlugin::output(std::string outfile) {
	res_no=1;
	if(opt<2){
...
PluginProxy<SlidingMinPDPlugin> SlidingMinPDPluginProxy = PluginProxy<SlidingMinPDPlugin>("SlidingMinPD", PluginManager::getInstance());
/* Convert any random array to heap array*/
void SlidingMinPDPlugin::heapify_mintaxa ( Mintaxa **list , int newnode )
{
	int done=0, dad;
	Mintaxa *temp;
...
/* Heap sort*/
void SlidingMinPDPlugin::heapsort_mintaxa ( Mintaxa **list, int last )
{
	int i;
	Mintaxa *temp;
	while(last > 0 )
...
/* Convert any random array to heap array*/
void SlidingMinPDPlugin::heapify_Fasta ( Fasta **list , int newnode )
{
	int done=0, dad;
	Fasta *temp;
...
/* Heap sort*/
void SlidingMinPDPlugin::heapsort_Fasta ( Fasta **list, int last )
{
	int i;
	Fasta *temp;
	while(last > 0 )
...
/*  																  */
/**********************************************************************/
void SlidingMinPDPlugin::AddTipOrNode(int *w,int *p,int *max_w, int chidx, double brlen,TNode **NodeStorage, int Join)
{
	int i;
	TNode *NewNode;
		if(*w<0||(*p<0&&Join>0))
...
/**********************************************************************/
double SlidingMinPDPlugin::GetAvgBrLen(TNode *node, int *nodes)
{
	double lenL, lenR;
...
/*  																  */
/**********************************************************************/
int SlidingMinPDPlugin::MatricizeTopology(TNode *node, int *topoMatrix, int n, int **dist, double avgBrLen,int *max)
{
...
/*  																  */
/**********************************************************************/
void SlidingMinPDPlugin::PrintTree(FILE *fv, TNode *node,Fasta **seqs,int *boots, int printboot)
{
	if (node->nodeNo>=1000) {
		fputc('(', fv);				
...
void SlidingMinPDPlugin::NJTree(TNode **NodeStorage, double ** DistMatrix, int UB, int OutgroupIn0, int *seqIds, int *max_w, int *w)
{
...
void SlidingMinPDPlugin::SetSeed(unsigned long seed)
{
	int N = 624;
...
//#endif
double SlidingMinPDPlugin::rndu()
{
	int N = 624, M=397;
    unsigned long y;
